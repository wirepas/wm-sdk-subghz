...
ifeq (,$(filter doxygen,$(MAKECMDGOALS)))
ifeq ($(target_board),)
...
# Folder for compressed archives of binaries
ARCHIVE_PATH := image/
...
# Board-specific files in archive are extracted here for processing
BUILDPREFIX_IMAGE := $(BUILDPREFIX_APP)image/

# Where to extract binaries from archive
EXTRACT_PATH := $(BUILDPREFIX_IMAGE)

# Where to search binaries from, in order
FIRMWARE_PATH := $(EXTRACT_PATH):$(IMAGE_PATH)
...
BL_UPDATER_BUILD_VARIABLES_MK := $(BUILDPREFIX_BL_UPDATER)bl_updater_build_variables.mk
