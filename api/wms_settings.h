...
#define APP_LIB_SETTINGS_VERSION 0x20a
...
/**
 * @brief Structure to hold key provisioning information for the settings API.
 *
 * This structure contains the key pairs and validity flags used for
 * network and management key provisioning.
 */
...
} app_lib_settings_key_provisioning_t;
 ...
 *
 * See @ref app_lib_settings_get_feature_lock_bits_f
 * "lib_settings->getFeatureLockBits"() for a description of feature lock bits.
...
/**
 * @brief Provision keys using the settings API.
 *
 * This function is reserved for future use
 *
 * @param[in] key_provisioning_p Pointer to the key provisioning information
 * structure. All fields must be filled in.
 *
 * @return Result code, @ref APP_RES_OK if successful,
 *         @ref APP_RES_INVALID_NULL_POINTER if key_info is NULL,
 *         or other error codes based on the provisioning result.
 */
 typedef app_res_e (*app_lib_settings_provision_keys_f)(
    const app_lib_settings_key_provisioning_t * key_provisioning_p);
