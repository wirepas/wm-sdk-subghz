 ...
 * @brief LL role bit
 *
 * This bit indicates if the role is a Low Latency (LL) role
 */
#define APP_LIB_SETTINGS_LL_ROLE_BIT 0x10
...
typedef app_res_e (*app_lib_settings_set_node_role_f)(
    ...
    /**
 * @brief Check if the authentication key is set and read the key value
 *
 * The return value indicates whether a valid key is set or not. If return
 * value is @ref APP_RES_OK then a value is written to the memory pointed
 * by the argument @p key_p. The value written to @p key_p is
 *
 *  - The authentication key, if the key management is NOT configured on the node, i.e.
 *    the management key pair is NOT set on the node.
 *  - Constant value 0xCC..CC, if the key management is configured, i.e.
 *    management key pair is set.
 *
 * See @ref app_lib_settings_key_management_config_f for more information about
 * setting the management key pair.
 *
 * @param   key_p
 *          A pointer to an array of 16 bytes where the key value is written.
 *          If NULL pointer is provided the key value is not written.
 * @return  Result code, @ref APP_RES_OK if a key is set,
 *          APP_RES_INVALID_CONFIGURATION if the key is all 0xff, i.e. not set
 */
...
/**
 * @brief Check if the encryption key is set and read the key value
 *
 * The return value indicates whether a valid key is set or not. If return
 * value is @ref APP_RES_OK then a value is written to the memory pointed
 * by the argument @p key_p. The value written to @p key_p is
 *
 *  - The encryption key, if the key management is NOT configured on the node, i.e.
 *    the management key pair is NOT set on the node.
 *  - Constant value 0xCC..CC, if the key management is configured, i.e.
 *    management key pair is set.
 *
