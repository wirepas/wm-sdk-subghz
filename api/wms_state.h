...
#define APP_LIB_STATE_VERSION 0x20F
...
typedef struct
{
    ...
    bool        is_da_support;
    /**
     * @brief   This beacon is secured with encryption
     */
    bool        is_secured;
...
/**
 * @brief       Stops an ongoing scan operation before it has ended.
 * @note        This can only be done on @ref APP_LIB_SETTINGS_ROLE_ADVERTISER
 *              "advertisers" and @ref APP_LIB_SETTINGS_ROLE_SUBNODE_LE
 *              "LE subnodes" (without autorole).
 * @return      One of the following values:
 *              - @ref APP_RES_OK
 *                - scan operation was stopped successfully
 *              - @ref APP_RES_INVALID_CONFIGURATION
 *                - device is not @ref APP_LIB_SETTINGS_ROLE_ADVERTISER or
 *                  @ref APP_LIB_SETTINGS_ROLE_SUBNODE_LE
 *              - @ref APP_RES_INVALID_STACK_STATE
 *                - stack is not running
 *
 * This service aborts an ongoing scan operation started with @ref
 * app_lib_state_start_scan_nbors_f "lib_state->startScanNbors()".
 */
