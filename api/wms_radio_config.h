 ...
 *
 * Library services are accessed  via [lib_radio_cfg](@ref app_lib_radio_cfg_t)
 * handle.
 *
 */
...
#define APP_LIB_RADIO_CFG_VERSION           0x201

/** @brief Maximum amount of configurable power levels */
#define APP_LIB_RADIO_CFG_POWER_MAX_CNT     14

/** @brief Minimum configured radio current (unit 100 µA = 0.1 mA) */
#define APP_LIB_RADIO_CFG_CURRENT_MIN       1

/** @brief Configure DECT Band 22 */
#define APP_LIB_DECT_BAND_22 (1UL<<22)


/**
 * @brief Maximum configured radio current (unit 100 µA = 0.1 mA)
 *
 ...
 * limit for current to be 2A.
 */
#define APP_LIB_RADIO_CFG_CURRENT_MAX       20000
 ...
 * It is set via [lib_radiocfg->femSetup()](@ref app_lib_radio_cfg_t) in
 ...
 * [lib_radiocfg->powerSetup()](@ref app_lib_radio_cfg_t) in
 * [power_cfg](@ref app_lib_radio_cfg_power_setup_f) argument.
...
/**
 * @brief   Set up radio band configuration, relevant only for DECT platforms.
 * @param   band_mask
 *          Bitmask of enabled DECT band. Bit position corresponds to the enabled band.
 * @note    This function is called before the stack is started to set the band configuration.
 *
 *          This function should not be called directly from the application code, but
 *          it is called by the application startup code before application initialization.
 *          Configuration is chipset-specific and handled by the build system.
 *          Calling this with a band not supported by the chipset will cause a reset when
 *          the stack is started. Note that this could lead to a reset-loop if such a
 *          call with unsupported band is hardcoded into your app.

 * @return  Result code, @ref APP_RES_OK if successful
 */

typedef app_res_e (*app_lib_radio_cfg_band_setup_f) (uint32_t band_mask);
