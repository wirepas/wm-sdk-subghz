...
#define APP_LIB_DATA_VERSION    0x211
 ...
 *
 * A result code returned from @ref app_lib_data_set_config_data_item_f
 * "lib_data->setConfigDataItem()" and for @ref
...
/**
 * \brief   Set the config data item of a specific endpoint
 *
 * \note If this function is called with length 0, the item is removed.
 * \note There is a maximum amount of items that can be set.
 * \note If the item is already set, it is overwritten.
 *
 * \param[in]   item
 *              Pointer to the config data item information
 *
 ...
 * \param[in,out]   item
 *                  Pointer to the config data item information where it is written.
 *                  \ref app_lib_config_data_item_t.bytes must be a valid pointer to a buffer
 *                  and \ref app_lib_config_data_item_t.endpoint must contain the endpoint to retrieve
 *                  \ref app_lib_config_data_item_t.length will be set based on the retrieved data.
 *
 * \return  Result code, @ref APP_LIB_DATA_CONFIG_DATA_RES_SUCCESS if
 *          successful.  See @ref app_lib_data_config_data_res_e for
 *          other result codes.
...
/**
 * \brief           Get all the config data endpoints
 * \param[out]      endpoints
 *                  Preallocated array of endpoints stored in the node with at least \p max_items items.
 * \param           max_items
 *                  Maximum number of endpoints that can be retrieved
 *
 * \return          Number of retrieved endpoints
 *
 * Example:
 * @code
 * #define MAX_ITEMS 6
...
/**
 * @brief   Disable boot diagnostics
 ...
 *          already.
 *
 * @note    In a network where many devices reboot at the same time it can be
 *          useful to disable boot diagnostics to avoid network congestion,
 *          the stack will then not send any boot diagnostics packets.
 *
 *          This function must be called at bootup of the device before the
 *          stack is started. It will prevent boot diagnostics to be sent.
 *          If the function is called after the boot diagnostics packet has
 *          already been sent, call will return @ref APP_RES_UNSPECIFIED_ERROR.
 *          Also if called multiple times, @ref APP_RES_UNSPECIFIED_ERROR is
 *          returned as well.
 *
 *          Warning: be aware this may make debugging problems in your network
 *          difficult. Use at own risk.
