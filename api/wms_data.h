 ...
 *
 * Library services are accessed  via [lib_data](@ref app_lib_data_t) handle.
 */
...
#define APP_LIB_DATA_VERSION    0x212
 ...
 *          may be used.
 *
 * Used with service [lib_data->sendData](@ref app_lib_data_send_data_f).
 */
 ...
 * [lib_data->getAppConfigNumBytes()](@ref app_lib_data_get_app_config_num_bytes_f) to get actual app config data size in
 * bytes, which may be smaller.
 *
 * Used with services  [lib_data->readAppConfig()](@ref app_lib_data_read_app_config_f), [lib_data->writeAppConfig()](@ref app_lib_data_write_app_config_f) and in callback function set with function
 * [lib_data->setNewAppConfigCb()](@ref app_lib_data_set_new_app_config_cb_f).
 */
 ...
 * packets.
 *
 * Used with service [lib_data->sendData()](@ref app_lib_data_send_data_f) and
 * callback functions set with services [lib_data->setDataReceivedCb()](@ref app_lib_data_set_data_received_cb_f).
 */
typedef enum
{
...
/**
 * @brief Flags to use with [lib_data->sendData()](@ref app_lib_data_send_data_f)
 */
typedef enum
{
     ...
     * [lib_data->setDataSentCb()](@ref app_lib_data_set_data_sent_cb_f)) when
...
/**
 * @brief A result code returned from [lib_data->sendData()](@ref app_lib_data_send_data_f)
 */
typedef enum
{
    ...
    APP_LIB_DATA_SEND_RES_INVALID_PARAM = 14,
 ...
 * functions set with services [lib_data->setDataReceivedCb()](@ref app_lib_data_set_data_received_cb_f).
 */
typedef enum
{
     ...
     * [lib_data->allowReception(true)](@ref app_lib_data_allow_reception_f) is
     * called. */
 ...
 *
 * A result code returned from [lib_data->readAppConfig()](@ref app_lib_data_read_app_config_f) and parameter for @ref
 ...
 *
 * Used with service [lib_data->setFragmentMode()](@ref app_lib_data_set_fragmented_mode_f)
 * to specify the mode
 */
typedef enum
{
 ...
 * services [lib_data->setDataReceivedCb()](@ref app_lib_data_set_data_received_cb_f)
 */
typedef struct
{
    /** Received bytes */
...
/**
 * @brief A struct for [lib_data->sendData()](@ref app_lib_data_send_data_f)
 */
typedef struct
{
    /** Bytes to send */
    ...
    /** Maximum value that should be used is 15. Do not use higher values than
      * that because that disables hop limiting */
    uint8_t hop_limit;
    /** Fragmentation information
 ...
 * [lib_data->setDataSentCb()](@ref app_lib_data_data_sent_cb_f)) when a packet
 ...
 * callback by calling either [lib_data->setDataReceivedCb()](@ref app_lib_data_set_data_received_cb_f).
 *
 ...
 * by calling [lib_data->setDataSentCb()](@ref app_lib_data_set_data_sent_cb_f).
 *
 * @param   status
 ...
 * \return  Result code, always @ref APP_RES_OK
 *
 * \warning This function will be deprecated in a future release. Use
 *          [lib_data->setConfigDataItemReceivedCb()](@ref app_lib_data_set_new_config_data_item_cb_f)
 *          instead.
 */
typedef app_res_e
...
/** Type for maximum data packet sizes that WM Stack can support.*/
typedef struct
{
 ...
 * [lib_data->getNumFreeBuffers()](@ref app_lib_data_get_num_free_buffers_f)
 ...
 * \note    When packets are send with [lib_data->sendData()](@ref app_lib_data_send_data_f) without tracking (tracking_id == @ref
 ...
 * Function [lib_data->getNumBuffers()](@ref app_lib_data_get_num_buffers_f)
 ...
 * is the same as calling [lib_data->allowReception(false)](@ref app_lib_data_allow_reception_f).
 *
 * Example of use:
 * @code
 ...
 *
 * \warning This function will be deprecated in a future release. Use
 *          lib_data->getConfigDataItem() to read the app config data.
 *
 * Example:
 * @code
 ...
 *
 * \warning This function will be deprecated in a future release. Use
 *          lib_data->setConfigDataItem() to set the app config data.
 *
 *
 * Example:
 * @code
...
/**
 * @brief Result of the config data item
 *
 * A result code returned from [lib_data->setConfigDataItem()](@ref app_lib_data_set_config_data_item_f) and for @ref
 * app_lib_data_get_config_data_item_f "lib_data->getConfigDataItem()"
 */
typedef enum
{
    /** Reading or writing the config data item was successful */
    APP_LIB_DATA_CONFIG_DATA_RES_SUCCESS = 0,
    /** Error: cannot write config data: node is not a sink */
    APP_LIB_DATA_CONFIG_DATA_RES_INVALID_ROLE = 1,
    /** Error: cannot read config data: no config data item set or received for
       that endpoint */
    APP_LIB_DATA_CONFIG_DATA_RES_INVALID_ENDPOINT = 2,
    /** Error: invalid NULL pointer parameter */
    APP_LIB_DATA_CONFIG_DATA_RES_INVALID_NULL_POINTER = 3,
    /** Error: out of memory */
    APP_LIB_DATA_CONFIG_DATA_RES_OUT_OF_MEMORY = 4,
    /** Error: invalid config data content */
    APP_LIB_DATA_CONFIG_DATA_RES_INVALID_CONTENT = 5,
    /** Error: invalid input or function arguments */
    APP_LIB_DATA_CONFIG_DATA_RES_INVALID_INPUT = 6,
   /** Error: feature is not supported */
    APP_LIB_DATA_CONFIG_DATA_RES_NOT_SUPPORTED = 7,
    /** Error: unable to access non-volatile memory */
    APP_LIB_DATA_CONFIG_DATA_RES_INTERNAL_ERROR = 8,
} app_lib_data_config_data_res_e;

/**
 * \brief Config Data Item information
 */
typedef struct
{
...
/**
 * \brief   Set the config data item of a specific endpoint. If a config data
 *          item is already set, it is overwritten.
 *
 * \details Each call of this function will trigger a transmission of all
 *          stored config data items to the nodes.
 *
 *          Specific endpoints are reserved for WM Stack use and have a well
 *          defined format for the payload data. Remaining endpoints are for
 *          application use. The number of config data items for application
 *          use is limited to 4 and the maximum size of the payload is 32 bytes.
 *          Refer to the WM Stack documentation for more details on endpoints.
 *
 *          This function is used also for removing a config data item, which
 *          is done by calling this function with length set to 0 and bytes set
 *          to NULL.
 *
 * \note    This API function can be succesfully called only on sink nodes.
  *
 * \param[in]   item    Pointer to the config data item information.
 *                      \ref app_lib_config_data_item_t.bytes must be a valid
 *                      pointer to a buffer or NULL pointer when removing a
 *                      config data item,
 *                      \ref app_lib_config_data_item_t.endpoint must contain
 *                      the endpoint to set or remove and
 *                      \ref app_lib_config_data_item_t.length must be set to
 *                      the length of the payload data or zero when removing a
 *                      config data item.
 *
 * \return  Result code, @ref APP_LIB_DATA_CONFIG_DATA_RES_SUCCESS if
 *          successful.  See @ref app_lib_data_config_data_res_e for
 *          other result codes.
 *
 * Example:
 * @code
 * uint8_t config_data_payload[APP_LIB_DATA_MAX_APP_CONFIG_NUM_BYTES];
 * app_lib_config_data_item_t config_data_item = {
 *      .bytes    = config_data_payload,
 *      .endpoint = 0x01,
 *      .length   = APP_LIB_DATA_MAX_APP_CONFIG_NUM_BYTES,
 * };
 *
 * lib_data->setConfigDataItem(&config_data_item);
 * @endcode
 */
typedef app_lib_data_config_data_res_e (*app_lib_data_set_config_data_item_f)(
    const app_lib_config_data_item_t * item);

/**
 * \brief           Get the config data item of a specific endpoint.
 *
 *
 * \param[in,out]   item
 *                  Pointer to the config data item information as follows:
 *                  \ref app_lib_config_data_item_t.bytes must be a valid
 *                  pointer to a buffer,
 *                  \ref app_lib_config_data_item_t.endpoint is the endpoint to
 *                  retrieve and
 *                  \ref app_lib_config_data_item_t.length is set to the
 *                  bytes buffers size when calling the function. The function
 *                  will set the length to the number of bytes written to the
 *                  buffer.
 *
 * \return  Result code, @ref APP_LIB_DATA_CONFIG_DATA_RES_SUCCESS if
 *          successful.  See @ref app_lib_data_config_data_res_e for
 ...
 *      .length   = APP_LIB_DATA_MAX_APP_CONFIG_NUM_BYTES,
 * };
 * lib_data->getConfigDataItem(&config_data_item);
 * @endcode
 */
typedef app_lib_data_config_data_res_e (*app_lib_data_get_config_data_item_f)(
    app_lib_config_data_item_t * item);


/**
 * \brief           Get the endpoints of all config data items stored on the
 *                  node.
 *
 * \param[out]      endpoints
 *                  Preallocated array to store the endpoint values of the
 *                  config data items on the node. Array size must be
 *                  at least \p max_items.
 * \param[in]       max_items
 *                  Maximum number of endpoints that can be retrieved
 *
 * \return          Number of endpoint values written into \p endpoints.
 *
 * \note            Note that the list of returned endpoints contain all
 *                  config data items, not limited to those used by the application,
 *                  but also including the endpoints used internally by the Wirepas stack.
 *
 * Example:
 * @code
 * #define MAX_ITEMS 6
 *
 * uint16_t endpoints[MAX_ITEMS];
 *
 * uint8_t amount_endpoints = lib_data->getAllConfigDataEndpoints(endpoints, MAX_ITEMS);
 * @endcode
 */
typedef uint8_t (*app_lib_data_get_all_config_data_endpoints_f)(uint16_t * endpoints, uint8_t max_items);

/**
 * \brief Callback to be notified when a new config data item is received
 *
 * New config data item callback is called whenever a new config data item is
 * received and when the node first joins a network. There is no return value
 * from the callback.
 *
 * The application sets a new app config callback by calling @ref
 * app_lib_data_new_config_data_item_cb_f "lib_data->setConfigDataItemReceivedCb()".
 *
 * \param   item
 *          New config data item
 */
typedef void (*app_lib_data_new_config_data_item_cb_f)(
    const app_lib_config_data_item_t * item);

/**
 * \brief   Set the callback function to be called when a new config data item is received
 *
 * \param   cb
 *          The function to be executed, or NULL to unset
 * \return  Result code, always @ref APP_RES_OK
...
typedef app_res_e
    ...
    (app_lib_data_new_config_data_item_cb_f cb);
 ...
 *
 * \warning This function will be deprecated in a future release. Use
 *          lib_data->setConfigDataItem() instead.
 *
 * Example:
 * @code
...
/**
 * @brief   Disable power on boot diagnostics
 * @return  @ref APP_RES_OK if successful, @ref APP_RES_UNSPECIFIED_ERROR if
 *          the boot diagnostics have already been disabled, or were created
 *          already.
 *
 * @note    In a network where many devices are powered on at the same time
 *          it can be useful to disable power on boot diagnostics to avoid
 *          network congestion, the stack will then not send power on boot
 *          diagnostics packets.
 *
 *          This function must be called at bootup of the device before the
 *          stack is started. It will prevent power on boot diagnostics to
 *          be sent. If the function is called after the boot diagnostics
 *          packet has already been sent, call will return
 *          @ref APP_RES_UNSPECIFIED_ERROR.
 *          Also if called multiple times, @ref APP_RES_UNSPECIFIED_ERROR is
 *          returned as well.
 *
 *          Consider asserting APP_RES_OK in test code.
 *
 * Example:
 * @code
 * void App_init(const app_global_functions_t* functions)
 * {
 *      lib_data->disableBootDiagnostics();
 *      // start the stack
 *      lib_state->startStack();
 * }
 * @endcode
 */
typedef app_res_e
    (*app_lib_data_disable_boot_diagnostics_f)(void);

/**
 * @brief   Dedicated value to disable local multicast functionality. To be used
 *          with service [lib_data->setLocalMulticastInfo()](@ref app_lib_data_set_local_mc_f)
 */
