...
HAS_BOOTLOADER=yes
# All EFR32 devices needs to have bootloader version 11 or newer
# * Series1 & Series2: Due enum size in HFXO/LFXO characteristics
# * Series2: Due default security model change
...
ifeq ($(MCU)$(MCU_SUB)$(MCU_MEM_VAR),efr32xg12pxxxf1024)
    ...
    # Different bootloader sizes available: size=mem_variant_byte,...
    BOOTLOADER_SIZES="32k=0x01"
    # By default, on xg23, do not use the dma for uart
    # as it can enter in conflict with external flash
    # Can still be explicitly enabled if needed
    uart_use_dma ?= no
    $(info No use of DMA for UART on xg23 by default)
else ifeq ($(MCU)$(MCU_SUB),efr32xg24)
    EFR32_SERIES=2
        # HWM_EFR32XG24       = 17, as hex string
        HW_MAGIC=11
        HW_VARIANT_ID_F1024=16
        HW_VARIANT_ID_F1536=17

    ifeq ($(MCU_MEM_VAR), xxxxf1536)
        HW_VARIANT_ID=$(HW_VARIANT_ID_F1536)
