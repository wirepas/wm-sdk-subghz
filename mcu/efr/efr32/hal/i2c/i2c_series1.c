...
static void i2c_bus_recovery(void)
{
     ...
     * m_i2c_bit_timeout_ns = 1.5 * (1/clock_hz) in ns.
     * Half period without margin ≈ m_i2c_bit_timeout_ns / 3 / 2,
     * but using /2000 (conservative) is fine for recovery. */
    ...
    for (int i = 0; i < 9; i++)
    {
        ...
        if (hal_gpio_get(BOARD_I2C_GPIO_PORT, BOARD_I2C_SDA_PIN))
        {
            break;
        }
    }
...
i2c_res_e I2C_init(i2c_conf_t * conf_p)
{
    ...
    I2C_IntClear(I2C_DEV, _I2C_IFC_MASK);
    ...
    // Configure the GPIOs first to avoid potential glitches
