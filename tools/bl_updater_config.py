...
def cmd_config_to_data(args):
    ...
    try:
        ...
        scratchpad_area = config.get_scratchpad_area()
    ...
    try:
        ...
        old_scratchpad_area = old_config.get_scratchpad_area()
    ...
    # Make sure the scratchpad area stays the same
    #
    # This is critical, as the new bootloader must be able to restore the
    # Wirepas Mesh stack firmware from the scratchpad written with the old
    # bootloader
    if (
        (scratchpad_area.address != old_scratchpad_area.address)
        or (scratchpad_area.length != old_scratchpad_area.length)
        or (scratchpad_area.flags != old_scratchpad_area.flags)
    ):
        raise ValueError("old and new bootloader scratchpad areas differ")
...
def create_argument_parser(pgmname):
    ...
    parser = argparse.ArgumentParser(
        ...
        description=textwrap.fill(
            "A tool to include a bootloader updater in a Wirepas Mesh stack firmware",
            help_width,
        ),
    )
