...
class NodeSettings:
    ...
    def to_bytes(self) -> bytes:
        ...
        output += self.node_address.to_bytes(4, byteorder='little') \
                    if self.node_address is not None else (0xffffffff).to_bytes(4, byteorder='little')
        output += NODE_ROLE[self.node_role].to_bytes(1, byteorder='little') \
                    if self.node_role is not None else (0xff).to_bytes(1, byteorder='little')
        output += self.diag_ival.to_bytes(2, byteorder='little') \
                    if self.diag_ival is not None else (0xffff).to_bytes(2, byteorder='little')
        output += (0xff).to_bytes(1, byteorder='little') # Padding bytes
...
class NetworkSettings:
    ...
    def to_bytes(self) -> bytes:
        ...
        output += self.net_addr.to_bytes(4, byteorder='little') \
                    if self.net_addr is not None else (0xffffffff).to_bytes(4, byteorder='little')
        output += self.net_ch.to_bytes(1, byteorder='little') \
                    if self.net_ch is not None else (0xff).to_bytes(1, byteorder='little')
        output += self.net_keys_seq.to_bytes(1, byteorder='little') \
                    if self.net_keys_seq is not None else (0xff).to_bytes(1, byteorder='little')
        output += self.mgmt_keys_seq.to_bytes(1, byteorder='little') \
                    if self.mgmt_keys_seq is not None else (0xff).to_bytes(1, byteorder='little')
        output += (0xff).to_bytes(1, byteorder='little') # Padding byte
...
class ProvisioningSettings:
    ...
    def to_bytes(self) -> bytes:
        ...
        output += PROV_METHOD[self.method].to_bytes(1, byteorder='little') \
                if self.method is not None else (0xff).to_bytes(1, byteorder='little')
        output += self.retries.to_bytes(1, byteorder='little') \
                if self.retries is not None else (0xff).to_bytes(1, byteorder='little')
        output += self.timeout.to_bytes(2, byteorder='little') \
                if self.timeout is not None else (0xffff).to_bytes(2, byteorder='little')
        ...
        output += len(binascii.unhexlify(uid_value)).to_bytes(1, byteorder='little') \
                if uid_value is not None else (0xff).to_bytes(1, byteorder='little')
...
class ActionSettings:
    ...
    def to_bytes(self) -> bytes:
        ...
        output += self.dualmcu_autostart.to_bytes(1, byteorder='little') \
                    if self.dualmcu_autostart is not None \
                    else (0xff).to_bytes(1, byteorder='little')
        output += self.preserve_data.to_bytes(1, byteorder='little') \
                    if self.preserve_data is not None \
                    else (0xff).to_bytes(1, byteorder='little')
        output += (0xffff).to_bytes(2, byteorder='little') # Padding
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--device',
                        choices = ['efr32xg12pxxxf512', 'efr32xg12pxxxf1024', 'efr32xg13pxxxf512',
                                  ...
                                  'nrf54l05', 'nrf54l10', 'nrf54l15', 'cc1354x10' ],
                        default = None,
                        help = 'device',
                        required=True,
                        type=str.lower)
