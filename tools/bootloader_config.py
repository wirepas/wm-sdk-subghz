...
def write_makefile_fragment(string_file, config, prefix=""):
    ...
    with open(string_file, "w") as f:
        vars = """prefix_bootloader_area_id=0x{:08x}
prefix_bootloader_area_addr=0x{:08x}
prefix_bootloader_area_length=0x{:08x}
prefix_bootloader_area_settings=0x{:08x}
prefix_stack_area_id=0x{:08x}
prefix_stack_area_addr=0x{:08x}
prefix_stack_area_length=0x{:08x}
prefix_erase_block_size=0x{:08x}
""".format(
            bootloader_area.id,
            bootloader_area.address,
            bootloader_area.length,
            bootloader_area.settings,
            wirepas_stack_area.id,
            wirepas_stack_area.address,
            wirepas_stack_area.length,
            eraseblock,
        )
        f.write(vars.replace("prefix_", prefix))
...
def create_argument_parser(pgmname):
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Prefix of make variable names (none by default)",
    )
...
def main():
    ...
    try:
        ...
        # Return stack area ID used in config
        if args.get_stack_area_id:
            try:
                print("0x%08x" % config.get_wirepas_stack_area().id)
            except AttributeError:
                raise ValueError("no stack area")

        # Return app area ID used in config
        if args.get_app_area_id:
            try:
                print("0x%08x" % config.get_app_area().id)
            except AttributeError:
                raise ValueError("no app area")
    ...
    if args.out_makefile:
        write_makefile_fragment(args.out_makefile, config, args.make_var_prefix)
