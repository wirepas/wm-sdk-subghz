...
class FirmwareConfig(object):
    ...
    def from_file(cls, file):
        ...
        return cls(general=general_conf, mem_layout=mem_layout,
                   libraries=libs, path=file)
 ...
 *     No firmware found fulfilling your requirements.
 *     radio_config=india865 has been defined. That has sometimes
 *     identified radio configuration with non-whitening and sometimes
 *     radio configuration with whitening. As in this release both are
 *     present at least in some extent, the selection between
 *     non-whitening and whitening needs to take place."""

error_no_firmware_found_india865_xg13 = """*     For EFR32xG13, only radio configuration with whitening is available,
 *     please configure radio_config=india865w."""

error_no_firmware_found_india865_xg23 = """*     For EFR32xG23, the selection of radio configuration between non-whitening
 *     and whitening needs to be done, thus please configure
 *     radio_config=india865nw or radio_config=india865w."""
...
def print_multiple_firmware_found(target_config, firmwares):
    ...
    for fw in firmwares:
        ...
        print_red(
            "*        {}: {}".format(fw.conf_path, fw.get_unspecified_keys(target_config))
        )
    print_red(stars_line)
    ...
    for directory in path_config["archive_path"].split(":"):
        ...
        for file in os.listdir(directory):
            ...
            if file.endswith(".zip"):
                ...
                extract_stack_zip(fullpath, mcu_full,
                                  path_config["extract_path"])
...
def select_most_recent_firmware(firmware_configs):

    highest_version_fw = firmware_configs[0]

    for fw in firmware_configs:

        if fw.general["version"] > highest_version_fw.general["version"]:

            highest_version_fw = fw

    print("Multiple versions available for {}{}, select: {}".format(
            highest_version_fw.general["mcu"],
            highest_version_fw.general["mcu_sub"],
            highest_version_fw.general["version"]
        )
    )

    return highest_version_fw


def select_firmware(path_config, target_config):

    # Get all firmware configurations matching the target_config
    firmware_configs = get_firmware_configs(path_config["firmware_path"],
                                            target_config)
    ...
    # Found more than one, select most recent version
        ...
        return select_most_recent_firmware(firmware_configs)
...
def verify_modem_archive(archive_fullpath, modem_md5):
    ...
    with open(archive_fullpath, 'rb') as archive_file:
        ...
        if archive_md5 != modem_md5:
            ...
            print("MD5 mismatch for {}. Expected: {}, got: {}"
                 .format(archive_fullpath, modem_md5, archive_md5))
...
def main():
    ...
    try:
        ...
        output_firmware(firmware_config, target_config, path_config,
