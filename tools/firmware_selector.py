...
class FirmwareConfig(object):
    ...
    def parse_version(self, version_str):
        """Numeric components (X.Y.Z.N) as a tuple of ints; suffix ignored.

        Throws ValueError if there is no numeric version to parse -- same spirit
        as the original int() failure, but on the part that actually matters.
        """

        # Match the leading run of dot-separated integers, e.g. "5.10.0.66".
        # Everything after it (".xxx", "yyy", "_@#$", "") is the
        # suffix and is discarded. The suffix can be missing or malformed without
        # affecting parsing. .match is anchored at the start; use .search instead if
        # versions can carry a non-digit prefix like "v5.10.0.66".
        match = re.compile(r"\d+(?:\.\d+)*").match(version_str.strip())
        if not match:
            raise ValueError("no numeric version in {!r}".format(version_str))
        return tuple(int(p) for p in match.group(0).split("."))   # add [:4] to hard-cap

    def is_version_greater_or_equal(self, min_version):
        current = self.parse_version(self.general["version"])
        minimum = self.parse_version(min_version)
        n = max(len(current), len(minimum))
        current += (0,) * (n - len(current))   # pad so 5.10 == 5.10.0
        minimum += (0,) * (n - len(minimum))
        if current >= minimum:
            return True
        print("Version for this binary is too old: {} < {}".format(
