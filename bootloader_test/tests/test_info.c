...
/** Size of memory area in BLCONFIG in bytes */
#define BLCONFIG_MEMORY_AREA_SIZE (16)

/** Encryption key length in bytes */
#define ENCRYPTION_KEY_LENGTH (16)

/** \brief  AES-128 key length */
#ifdef BOOT_TEST_OMAC1_AES128CTR
/** Authentication key length in bytes */
#define AUTH_KEY_LENGTH  (16)

/** Number of memory areas in BLCONFIG */
#define NUM_MEMORY_AREAS (8)

/** Size of BLCONFIG in bytes */
#define BLCONFIG_NUM_BYTES (1024)

/** Number of authentication/encryption key pairs in BLCONFIG */
#define NUM_KEY_PAIRS                                                          \
    ((BLCONFIG_NUM_BYTES - NUM_MEMORY_AREAS * BLCONFIG_MEMORY_AREA_SIZE)       \
     / (AUTH_KEY_LENGTH + ENCRYPTION_KEY_LENGTH))
#else
/** Authentication key length in bytes */
#define AUTH_KEY_LENGTH  (64)

/** Number of memory areas in BLCONFIG */
#define NUM_MEMORY_AREAS (16)

/** Number of authentication/encryption key pairs in BLCONFIG */
#define NUM_KEY_PAIRS    (8)
#endif

/** Authentication/encryption key pair length in bytes */
#define KEY_PAIR_LENGTH (AUTH_KEY_LENGTH + ENCRYPTION_KEY_LENGTH)


bool info_keys(const memory_area_services_t * mem_area_services)
{
    (void) mem_area_services;

    bool res = true;
    const uint8_t * keys
        = (const uint8_t *) (BOOTLOADER_ADDRESS + BLCONFIG_OFFSET
                             + NUM_MEMORY_AREAS * BLCONFIG_MEMORY_AREA_SIZE);

    START_TEST(INFO, List all keys stored in bootloader);
    for(uint32_t i=0; i < NUM_KEY_PAIRS; i++)
    {
        const uint8_t * cur_key = keys + i * KEY_PAIR_LENGTH;
        ...
        for (uint32_t j = 0; j < AUTH_KEY_LENGTH; ++j)
        {
            Print_printf("[%02X]", cur_key[j]);
        }

        Print_printf("\n");

        cur_key += AUTH_KEY_LENGTH;
        Print_printf("Encryption     key n° %02d : ", i);
        for (uint32_t j = 0; j < ENCRYPTION_KEY_LENGTH; ++j)
        {
            Print_printf("[%02X]", cur_key[j]);
        }
...
bool info_flash(const memory_area_services_t * mem_area_services)
{
    ...
    res = search_ext_flash_area(mem_area_services, &id);
    if (id != BL_MEMORY_AREA_UNDEFINED)
    {
