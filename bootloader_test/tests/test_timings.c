...
#if defined(NRF52_SERIES) || defined(NRF54L_SERIES) || defined(NRF91_SERIES)
    #define MCU_FREQ_MHZ 64
#elif defined(CC13X4_PLATFORM)
    #define MCU_FREQ_MHZ 48
#else //EFR32
                         ...
                         bl_memory_area_info_t * area_info_p)
{
    ...
    if (area_info_p->flash.erase_sector_size == 0)
    {
        /* NVM doesn't need erasing -> set number of sectors to 1 in order to
         * get some function call timings. */
        number_of_sector = 1;
    }
    else
    {
...
bool Tests_timings(bl_interface_t * interface)
{
    ...
    res = search_ext_flash_area(interface->memory_area_services_p, &id);
    if (id != BL_MEMORY_AREA_UNDEFINED)
    {
        res |= test_timings(id, &info);
    }
