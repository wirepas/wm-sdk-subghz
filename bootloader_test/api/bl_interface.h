...
typedef enum
{
    ...
    BL_RES_NODRIVER      = 3,  ///< Missing flash driver, hw error, verify error
    BL_RES_PARAM         = 4,  ///< Invalid parameters
    BL_RES_INVALID_AREA  = 5,  ///< Area doesn't exist
    BL_RES_TIMEOUT       = 6,  ///< Synchronous call timeout
    BL_RES_INVALID_STATE = 7,  ///< Write ongoing or scratchpad not valid
    BL_RES_INVALID_TAG   = 8,  ///< Record with tag doesn't exist, or no space
} bl_interface_res_e;

/** \brief  Value of an area that doesn't exist */
...
typedef enum
{
    ...
    BL_MEM_AREA_TYPE_MODEMDELTAFW = 7,  ///< Modem delta firmware area
...
typedef struct
{
    ...
    uint16_t crc;
    /** Sequence number of data in scratchpad: \ref BL_SCRATCHPAD_MIN_SEQ ..
     * \ref BL_SCRATCHPAD_MAX_SEQ */
    uint8_t seq;
...
{
    /** Full modem FW image */
    BL_SCRAT_MODEM_FILE_TYPE_FULL_IMAGE = 0,
    /** Delta modem FW image */
    BL_SCRAT_MODEM_FILE_TYPE_DELTA_IMAGE = 1,
    /** Invalid image */
    BL_SCRAT_MODEM_FILE_TYPE_INVALID = 255
...
typedef struct
{
    ...
    bool dedicated;
    /** Type of the modem firmware present in scratchpad
...
typedef union
{
     ...
     */
    const bl_platform_nrf54_t * nrf54;
     ...
     *  (introduced in bootloader v14).
     */
    const bl_platform_cc13x4_t * cc13x4;
#endif
} bl_platform_t;
 ...
 */
typedef bl_interface_res_e (*bl_memory_area_startRead_f)(bl_memory_area_id_t id,
                                                         void *              to,
                                                         uint32_t from,
                                                         size_t   amount);
 ...
 *         See \ref bl_interface_res_e for other result codes.
 */
typedef bl_interface_res_e (*bl_memory_area_startWrite_f)(
    bl_memory_area_id_t id, uint32_t to, const void * from, size_t amount);
 ...
 *         See \ref bl_interface_res_e for other result codes.
 */
typedef bl_interface_res_e (*bl_memory_area_startErase_f)(
    bl_memory_area_id_t id, uint32_t * sector_base, size_t * number_of_sector);
 ...
 *         See \ref bl_interface_res_e for other result codes.
 */
typedef bl_interface_res_e (*bl_memory_area_getAreaInfo_f)(
    bl_memory_area_id_t id, bl_memory_area_info_t * info);
 ...
 * \return none
 */
typedef void (*bl_memory_area_getAreaList_f)(bl_memory_area_id_t * list,
                                             uint8_t *             num_areas);
 ...
 *         See \ref bl_interface_res_e for other result codes.
 */
typedef bl_interface_res_e (*bl_memory_area_getIdfromType_f)(
    bl_memory_area_id_t * id, bl_memory_area_type_e type);
 ...
 */
typedef bl_interface_res_e (*bl_memory_area_getAreaHeader_f)(
    bl_memory_area_id_t id, bl_memory_area_header_t * header);
 ...
 *         See \ref bl_interface_res_e for other result codes.
 */
typedef bl_interface_res_e (*bl_scrat_getValidity_f)(
    bl_scrat_valid_e * validity);
 ...
 *         See \ref bl_interface_res_e for other result codes.
 */
typedef bl_interface_res_e (*bl_scrat_read_f)(void *   to,
                                              uint32_t from,
                                              size_t   amount);
 ...
 */
typedef bl_interface_res_e (*bl_scrat_begin_f)(uint32_t num_bytes,
                                               uint8_t  seq,
                                               void (*clear_wdt)(void));
 ...
 *         See \ref bl_interface_res_e for other result codes.
 */
typedef bl_interface_res_e (*bl_scrat_write_f)(
    uint32_t                  to,
    const void *              from,
    size_t                    amount,
 ...
 */
typedef const bl_hardware_capabilities_t * (*bl_hardware_getCapabilities_f)(
    void);
...
/**
 * \brief   Global interface entry point with a bootloader version number
 */
typedef struct
{
    uint32_t                       version;
