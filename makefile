...
# Replace the stack with Generic Bootloader Updater Tool, if requested
bl_updater ?= no
bl_updater_old_key_file ?= $(KEY_FILE)
bl_updater_match_app_area_id ?= no
bl_updater_reset_areas ?= no
ifneq ($(filter-out no,$(bl_updater)),)
# If Generic Bootloader Updater Tool is requested, the parameter bl_updater
# specifies the old bootloader size, e.g., "16k" or "32k"
#
# MODIFIED_STACK_HEX is the stack hex file that contains the bootloader updater,
...
MODIFIED_STACK_HEX := $(BL_UPDATER_HEX)
SCRATCHPAD_CONFIG_INI := $(BL_UPDATER_OLD_CONFIG_INI)
CLEAN += $(BL_UPDATER_HEX)
else
...
# If MCU is defined, check that files behind INI_FILE_WP and INI_FILE_APP are found.
# (MCU is not defined to targets like doxygen)
ifneq ($(MCU),)
ifeq ("$(wildcard $(INI_FILE_WP))","")
$(error INI_FILE_WP not found: $(INI_FILE_WP))
endif
ifeq ("$(wildcard $(INI_FILE_APP))","")
$(error INI_FILE_APP not found: $(INI_FILE_APP))
endif
endif
...
define SAVE_MAKE_PARAMS
    ...
    echo "bl_updater_reset_areas=$(bl_updater_reset_areas)" >> $(1); \
...
$(BL_UPDATER_HEX): $(BL_UPDATER_ENABLED) $(STACK_HEX) $(BOOTLOADER_HEX) $(APP_HEX) $(bl_updater_old_key_file)
	...
	$(DD)$(MAKE) \
	    ...
	    stack_area_id=$(stack_area_id) \
	    bl_updater_reset_areas=$(bl_updater_reset_areas) \
	    app_area_id=$(app_area_id)

$(BL_UPDATER_OLD_CONFIG_INI): $(BL_UPDATER_HEX)

$(BL_UPDATER_BUILD_VARIABLES_MK): $(BL_UPDATER_HEX)
...
otap_all otap_app_only: initial_setup $(SCRATCHPAD_CONFIG_INI)
