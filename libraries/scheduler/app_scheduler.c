...
#define DEBUG_LOG_MODULE_NAME "SCHED LIB"
...
static void perform_task(task_t * task)
{
    ...
    app_lib_time_timestamp_hp_t begin_hp=lib_time->getTimestampHp();

    // Execute the task selected
    next = task_cb();

    // Check time spent in the task and compare with the time allowed.
    app_lib_time_timestamp_hp_t end_hp=lib_time->getTimestampHp();
    uint32_t cpuTime = lib_time->getTimeDiffUs(end_hp, begin_hp);
    if (cpuTime > task->exec_time_us)
    {
        LOG(LVL_WARNING, "task take too much time 0x%08x:(%d > %d)",task_cb,cpuTime,task->exec_time_us);
    }
