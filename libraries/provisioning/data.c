...
/**
 * \brief   Parse a 16-byte AES key from a CBOR byte-string into dst, set flag.
 * \param   value
 *          Pointer to a CBOR Value.
 * \param   dst
 *          Destination buffer (APP_LIB_SETTINGS_AES_KEY_NUM_BYTES bytes).
 * \param   flag
 *          Pointer to the boolean flag to set on success.
 * \return  A CborError error code.
 */
static CborError parse_key_bytes(CborValue * value, uint8_t * dst, bool * flag)
{
    CborError err;
    size_t len = APP_LIB_SETTINGS_AES_KEY_NUM_BYTES;
    err = extract_byte_string(value, dst, &len);
    if (err != CborNoError)
    {
        return err;
    }
                                    ...
                                    int id)
{
    ...
    switch(id)
    {
        ...
        {
            err = parse_key_bytes(value,
                                  m_provisioning_data.enc_key,
                                  &m_provisioning_data.is_enc_key_set);
            break;
        }
        case PROV_DATA_ID_AUTH_KEY:
        {
            err = parse_key_bytes(value,
                                  m_provisioning_data.auth_key,
