 ...
 *          - Non‑NULL pointer.
 *          - Valid provisioning method (one of provisioning_method_e).
    ...
    const provisioning_settings_t* settings)
{
    ...
    // Valid provisioning method?
    switch (settings->method)
    {
        case PROV_METHOD_UNSECURED:
        case PROV_METHOD_SECURED:
        case PROV_METHOD_EXTENDED_UID:
        case PROV_METHOD_EXTENDED_UID_KEY_MGMT:
            break;
        default:
            return SETTINGS_INVALID_METHOD;
    }
