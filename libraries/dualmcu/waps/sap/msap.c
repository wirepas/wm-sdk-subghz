...
/**
 * \brief   List config data items request
 * \param   item
 *          Item containing WAPS frame
 * \return  true, if frame was accepted
 */
static bool configDataListItems(waps_item_t *item);

/**
 * \brief   Set config data item request
 * \param   item
 *          Item containing WAPS frame
 * \return  true, if frame was accepted
 */
static bool configDataItemSet(waps_item_t *item);
...
bool Msap_handleFrame(waps_item_t * item)
{
    switch (item->frame.sfunc)
    {
        ...
        case WAPS_FUNC_MSAP_CONFIG_DATA_ITEM_SET_REQ:
            return configDataItemSet(item);
        case WAPS_FUNC_MSAP_CONFIG_DATA_ITEM_GET_REQ:
...
static bool configDataItemSet(waps_item_t *item)
{
    msap_config_data_item_set_req_t req = item->frame.msap.config_data_item_set_req;
    uint8_t result = MSAP_CONFIG_DATA_ITEM_SET_SUCCESS;

    app_lib_config_data_item_t cdc_item = {
        .endpoint = req.npd_endpoint,
        .length = req.npd_payload_len,
        .bytes = req.npd_payload,
    };

    app_lib_data_config_data_res_e res = lib_data->setConfigDataItem(&cdc_item);

    Waps_item_init(item,
                   WAPS_FUNC_MSAP_CONFIG_DATA_ITEM_SET_CNF,
                   sizeof(msap_config_data_item_set_cnf_t));

    switch (res)
    {
    case APP_LIB_DATA_CONFIG_DATA_RES_SUCCESS:
        result = MSAP_CONFIG_DATA_ITEM_SET_SUCCESS;
        break;
    case APP_LIB_DATA_CONFIG_DATA_RES_INVALID_ROLE:
        result = MSAP_CONFIG_DATA_ITEM_SET_INVALID_ROLE;
        break;
    case APP_LIB_DATA_CONFIG_DATA_RES_INVALID_ENDPOINT:
        result = MSAP_CONFIG_DATA_ITEM_SET_INVALID_ENDPOINT;
        break;
    case APP_LIB_DATA_CONFIG_DATA_RES_OUT_OF_MEMORY:
        result = MSAP_CONFIG_DATA_ITEM_SET_OUT_OF_MEMORY;
        break;
    case APP_LIB_DATA_CONFIG_DATA_RES_INVALID_CONTENT:
        result = MSAP_CONFIG_DATA_ITEM_SET_INVALID_CONTENT;
        break;
    case APP_LIB_DATA_CONFIG_DATA_RES_NOT_SUPPORTED:
    default:
        result = MSAP_CONFIG_DATA_ITEM_SET_NOT_SUPPORTED;
        break;
    }

    item->frame.msap.config_data_item_set_cnf.result = result;
...
static bool configDataListItems(waps_item_t *item)
{
    uint16_t endpoints[MSAP_CONFIG_DATA_LIST_ITEMS_MAX_NUM_ENDPOINTS];
    uint8_t num_endpoints = 0;
    msap_config_data_list_items_e result = MSAP_CONFIG_DATA_LIST_ITEMS_SUCCESS;
    num_endpoints = lib_data->getAllConfigDataEndpoints(endpoints, MSAP_CONFIG_DATA_LIST_ITEMS_MAX_NUM_ENDPOINTS);

    if (num_endpoints == UINT8_MAX)
    {
        // num_endpoints == UINT8_MAX is a special value indicating that the feature is not supported
        num_endpoints = 0;
        result = MSAP_CONFIG_DATA_LIST_ITEMS_NOT_SUPPORTED;
    }
    else if (num_endpoints > MSAP_CONFIG_DATA_LIST_ITEMS_MAX_NUM_ENDPOINTS)
    {
        // This is very wrong, likely the endpoint values are not sensible either
        result = MSAP_CONFIG_DATA_LIST_ITEMS_SUCCESS;
        num_endpoints = 0;
    }

    Waps_item_init(item,
        WAPS_FUNC_MSAP_CONFIG_DATA_LIST_ITEMS_CNF,
        MSAP_CONFIG_DATA_LIST_ITEMS_HEADER_SIZE + num_endpoints * sizeof(uint16_t));

    item->frame.msap.config_data_list_items_cnf.result = result;
    item->frame.msap.config_data_list_items_cnf.amount_endpoints = num_endpoints;
    memcpy(item->frame.msap.config_data_list_items_cnf.endpoints, endpoints, num_endpoints * sizeof(uint16_t));
...
static bool configDataItemGet(waps_item_t *item)
{
    ...
    uint8_t bytes[MSAP_NPD_PAYLOAD_MAX_LEN];

    app_lib_config_data_item_t cdc_item = {
        .bytes = bytes,
        .endpoint = endpoint,
        .length = MSAP_NPD_PAYLOAD_MAX_LEN,
    };

    app_lib_data_config_data_res_e res = lib_data->getConfigDataItem(&cdc_item);

    if (res == APP_LIB_DATA_CONFIG_DATA_RES_SUCCESS)
    {
        Waps_item_init(item,
                       WAPS_FUNC_MSAP_CONFIG_DATA_ITEM_GET_CNF,
                       MSAP_CONFIG_DATA_ITEM_GET_HEADER_SIZE + cdc_item.length);
        memset(item->frame.msap.config_data_item_get_cnf.npd_payload, 0, MSAP_NPD_PAYLOAD_MAX_LEN);

        memcpy(item->frame.msap.config_data_item_get_cnf.npd_payload,
               cdc_item.bytes,
               cdc_item.length);

        item->frame.msap.config_data_item_get_cnf.result = MSAP_CONFIG_DATA_ITEM_GET_SUCCESS;
        item->frame.msap.config_data_item_get_cnf.npd_payload_len = cdc_item.length;
    }
    else
    {
        Waps_item_init(item,
        ...
        if (res == APP_LIB_DATA_CONFIG_DATA_RES_INVALID_ENDPOINT)
        {
            item->frame.msap.config_data_item_get_cnf.result = MSAP_CONFIG_DATA_ITEM_GET_INVALID_ENDPOINT;
        }
        else
        {
            item->frame.msap.config_data_item_get_cnf.result = MSAP_CONFIG_DATA_ITEM_GET_NOT_SUPPORTED;
        }
        item->frame.msap.config_data_item_get_cnf.npd_payload_len = 0;
    }
                    ...
                    waps_item_t * item)
{
    ...
    Waps_item_init(item,
                   ...
                   sizeof(msap_config_data_item_ind_t));

    item->time = 0;
    item->frame.sfid = 0;

    msap_config_data_item_ind_t * ind = &item->frame.msap.config_data_item_ind;
    ind->npd_payload_len = cdc_item->length;
    ind->npd_endpoint = cdc_item->endpoint;
    ind->queued_indications = 0;
