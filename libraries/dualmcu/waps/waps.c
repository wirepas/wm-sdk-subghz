...
/**
 * Time interval of the periodic maintenance task.
 */
#define WAPS_MAINTENANCE_TASK_PERIOD_MS               5000

/** Value by which the gateway inactivity counter is incremented
 */
#define WAPS_GATEWAY_INACTIVITY_COUNTER_INCR_MS    WAPS_MAINTENANCE_TASK_PERIOD_MS

/** The maximum value that the gateway inactivity counter can
 * increase to.
 */
#define WAPS_GATEWAY_INACTIVITY_COUNTER_MAX_MS       60000
 ...
 *          Every period, the queued indications will be compared to the max
 *          TTL set in the node to removed if too old.
 */
static void garbage_collect_old_indications(void);

/**
 * \brief   Run periodic maintenance
 * \details Run indication queue carbage collection and gateway
 *          inactivity monitoring.
 */
static uint32_t periodic_maintenance_task(void);

/**
 * \brief   Callback when a new config data item is received
 * \param   cdc_item
 *          Pointer to the config data item
 */
static void config_data_item_cb(const app_lib_config_data_item_t * cdc_item);

/**
 * \brief   Updates the gateway inactivity counter if gateway activity was not
 *          observed or reset it otherwise.
 * \param   gateway_activity_signal
 *          true if gateway activity was observed and false otherwise
 */
static void gateway_inactivity_monitoring(bool gateway_activity_signal);
...
// Signal gateway activity. Any packet received from gateway writes this signal to
// one. Signal is reset once periodic maintenance task is run.
static bool m_signal_gateway_activity;

// Counter that is incremented if there is no gateway activity during time
// interval of WAPS_MAINTENANCE_TASK_PERIOD_MS (milliseonds). The counter is
// reset immediately if there is gateway activity.
static uint32_t gateway_inactivity_sink_ctr;
...
bool Waps_init(uint32_t baudrate, bool flow_ctrl)
{
    ...
    if (Waps_prot_init(receive_request, baudrate, flow_ctrl))
    {
        ...
        m_signal = 0;
        m_signal_gateway_activity = false;

        // Start a task to remove old indication (according to TTL)
        App_Scheduler_addTask_execTime(periodic_maintenance_task, WAPS_MAINTENANCE_TASK_PERIOD_MS, 100);
        return true;
    }
...
void wakeup_task(void)
{
    ...
    m_signal_gateway_activity = true;
}
...
static uint32_t periodic_maintenance_task(void)
{
    // Run carbage collection
    garbage_collect_old_indications();

    // Check gateway inactivity
    gateway_inactivity_monitoring(m_signal_gateway_activity);
    m_signal_gateway_activity = false;

    return WAPS_MAINTENANCE_TASK_PERIOD_MS;
}
...
static void gateway_inactivity_monitoring(bool gateway_activity_signal)
{
#ifdef GATEWAY_MONITORING

    if (gateway_activity_signal)
    {
        if(gateway_inactivity_sink_ctr > 0)
        {
            lib_state->setSinkCost(0);
        }
        gateway_inactivity_sink_ctr = 0;
    }
    else
    {
        // If counter is less than max value, then increment
        if (gateway_inactivity_sink_ctr < WAPS_GATEWAY_INACTIVITY_COUNTER_MAX_MS)
        {
            gateway_inactivity_sink_ctr += WAPS_GATEWAY_INACTIVITY_COUNTER_INCR_MS;
            // set sink route state to false if max value was crossed
            if (gateway_inactivity_sink_ctr >= WAPS_GATEWAY_INACTIVITY_COUNTER_MAX_MS)
            {
                gateway_inactivity_sink_ctr = WAPS_GATEWAY_INACTIVITY_COUNTER_MAX_MS;
                lib_state->setSinkCost(APP_LIB_STATE_INVALID_ROUTE_COST);
            }
        }
    }
#else
    (void)gateway_activity_signal;
#endif
