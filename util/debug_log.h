...
/******************************************************************************
 * DEFINES - LOGGING BACKEND
 *
 * Three backends supported:
 *   1. USE_DEBUG_PRINT_MODULE - Internal debug print module(rtt, uart, multi uart)
 *   2. APP_PRINTING           - App printing via UART
 *   3. (none)                 - No-op (logging disabled)
 ******************************************************************************/
...
/* Active wait for UART buffer to flush */
#define Flush_Log()                                                           \
    do {                                                                      \
        app_lib_time_timestamp_hp_t _end;                                     \
        _end = lib_time->addUsToHpTimestamp(lib_time->getTimestampHp(),       \
                                            FLUSH_DELAY_MS * 1000);           \
...
#endif /* USE_DEBUG_PRINT_MODULE / APP_PRINTING */
...
{ \
    ...
    Print_Log( \
        ...
        fmt S_LINEENDING S_TIME_SUFFIX S_FUNCTION_SUFFIX S_LINE_SUFFIX \
        , ##__VA_ARGS__) : \
    (void)NULL; \
}
